...
load_dotenv()
chunk_path = os.getenv('DATA_PATH_JSON')
...
codigoUsuario = '112448' #os.getenv('USER_ID_ELOPAR')
...
for index,row in df.iterrows():
    ...
    soap_request_instance = SOAPRequest_downloadArquivosPorGed(codigoEmpresa, chaveAcesso, codigoEmpresaPrincipal,
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            file_downloaded_name = soap_request_instance.extract_and_save_files(response.content, 'E:\\RAIA_DROGASIL\\IMAGENS_GED')
            ...
            print(f'FILE DOWNLOADED: {file_downloaded_name} ITEM: {index} of {len(df)}')
            
            df.at[index,'BAIXADO'] = 'OK'
            df.to_json(f'{chunk_path}', orient='records')
        else:
